...
class VcnetAtt(nn.Module):
    def __init__(self, cfg_density, num_grid, cfg, degree, knots, att_layers=1):
        ...
        for layer_idx, layer_cfg in enumerate(cfg):
            if layer_idx == len(cfg) - 1:  # last layer
                last_layer = Att_FC(layer_cfg[0], layer_cfg[1], self.degree, self.knots, act=layer_cfg[3],
                                    isbias=layer_cfg[2], islastlayer=1, treat_emb=self.treat_emb)
            else:
                blocks.append(
                    Att_FC(layer_cfg[0], layer_cfg[1], self.degree, self.knots, act=layer_cfg[3], isbias=layer_cfg[2],
                           islastlayer=0, treat_emb=self.treat_emb))
        blocks.append(last_layer)
...
class VcnetAttv2(nn.Module):
    def __init__(self, embed_size=10, num_t=1, num_cov=25, num_heads=2, att_layers=1, dropout=0.1, init_range_f=0.1,
                 init_range_t=0.2):
        ...
        encoder_layers = TransformerEncoderLayer(embed_size, nhead=num_heads, dim_feedforward=50, dropout=dropout,
                                                 num_cov=num_cov)
        self.encoder = TransformerEncoder(encoder_layers, att_layers)

        decoder_layers = TransformerDecoderLayer(embed_size, nhead=num_heads, dim_feedforward=50, dropout=dropout,
                                                 num_t=num_t)
