...
class dr_cfr(object):
    ...
    def _build_graph(self, x, t, y_, dims, do_in, do_out, p_t, FLAGS, pi_0):
        ...
        y, weights_out, weights_pred, biases_out, bias_pred = self._build_output_graph(tf.concat([h_rep_norm_B, h_rep_norm_C], 1), t, 2*dim_in, dim_out, do_out, FLAGS)
    ...
    def _build_treatment_graph(self, h_rep_norm, dim_in):
        ...
        cost = -tf.reduce_mean( tf.multiply(t, tf.log(sigma)) + tf.multiply(1.0-t, tf.log(1.0-sigma)) ) + 1e-3*tf.nn.l2_loss(W)
