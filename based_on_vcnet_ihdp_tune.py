...
def criterion(out, y, alpha=0.5, beta=0, gamma=0.5, epsilon=1e-6):
    ...
    return total_loss, (
                (out[1].squeeze() - y.cuda().squeeze()) ** 2).mean(), treatment_loss, discrepancy_loss, imbalance_loss
...
if __name__ == "__main__":
    ...
    # hyperparameter
    parser.add_argument('--alpha', type=float, default=0.2, help='weight for treatment loss')
    parser.add_argument('--beta', type=float, default=0.2, help='weight for discrepency loss')
    parser.add_argument('--gamma', type=float, default=0.2, help='weight for imbalance loss')
    ...
    for model_name in ['Vcnet_disentangled']:
        ...
        Result["hyperparameter"]=[alpha,beta,gamma,init_lr]
            #
        with open(save_path + '/result_ivc_tune_20.json', 'a') as fp:
            json.dump(Result, fp)